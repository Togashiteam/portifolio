...
export default function BasicCalc() {
  ...
  const renderGridItems = () => {
    ...
    return gridValues.map((value) => {
      let className = "rounded-md py-1 border-2 border-success-700/50 flex justify-center items-center";
    ...
    <div className="flex min-h-screen flex-col items-center justify-between p-24">
      <div className="bg-success-300 rounded-lg px-2 pt-2 pb-4 space-y-6">
        <div className="flex align-middle h-16 text-xl bg-success-300">
          <input autoFocus={true} readOnly={true} className=' px-2 text-dark-700  rounded-t-lg rounded-b-md text-right focus-visible:outline focus-visible:outline-2 focus-visible:outline-success-700' type="text" value={inputValue} onChange={e => setInputValue(e.currentTarget.value)} />
        </div>
        <div>
